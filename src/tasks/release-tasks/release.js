...
const releaseTasks = version =>
  new Listr([
    ...
    },
    {
      ...
      task: () =>
        execa.shell(`git commit -m "${version}"`).catch(() => {
    ...
    },
    {
      ...
      task: () =>
        execa.shell(`git tag -a v${version} -m ${version}`).catch(() => {
          throw new Error(
            `Error: could not tag with v${version}... make sure this version is not tagged already`
          );
        })
    },
    {
      ...
      task: () =>
        execa.shell("git push --follow-tags").catch(() => {
          throw new Error(
            "Error: could not push to remote repository... make sure you have access to it"
          );
        })
    }
  ]);
